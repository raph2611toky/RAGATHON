...
def load_pdf(file_path: str) -> Tuple[List[str], List[Dict]]:
    """
    Reads text content from a PDF file using pdfplumber, handles tables and crops pages into halves.

    Args:
        file_path (str): Path to the PDF file.

    Returns:
        Tuple[List[str], List[Dict]]: List of page texts (including table markdown) and metadata.
    """
...
#============================Load Data============================#
def load_data(documents: List[str], metadatas: List[Dict], collection_name: str) -> chromadb.Collection:
    """
...
def get_relevant_passage(query: str, db: chromadb.Collection, n_results: int = 1) -> Tuple[str, Dict]:
    """
    ...
    Returns:
        Tuple[str, Dict]: Relevant document and its metadata.
    """
    results = db.query(
...
def classify_q(q: str) -> str:
    """
    Classifies the question type for response formatting.

    Args:
        q (str): The question text.

    Returns:
        str: Type ('num', 'pct', 'success', 'gen').
    """
    q = q.lower()
...
def extract_num(ans: str, q_type: str) -> str:
    """
    Extracts and formats numbers from the answer based on question type.

    Args:
        ans (str): The answer text.
        q_type (str): The classified question type.

    Returns:
        str: Formatted extracted value.
    """
    ans, ans_ = [ans.strip()] * 2
    ans = re.sub(r'\s+', '', ans)
    if q_type == "num" and ans.isnumeric():
        return f"{int(ans):,}".replace(",", " ")
    if (q_type in ["pct", "success"]) and ans.isnumeric():
        value = int(ans)
        if value > 100:
            numbers = re.findall(r'\b\d+\b(?=%|\s)', ans_)
    ...
    numbers = re.findall(r'\b\d+\b', re.sub(r'\s+', '', ans_))
    if not numbers:
        return ans
    ...
    if non_years:
        if q_type == "num":
            return f"{int(non_years[0]):,}".replace(',', " ") if len(non_years) == 1 else f"{int(non_years[-1]):,}".replace(',', " ")
...
def make_rag_prompt(query: str, relevant_passage: str) -> str:
    """
    Builds a RAG prompt from query and relevant passage, with type-specific instructions.

    Args:
        query (str): User query.
        relevant_passage (str): Relevant passage.
...
def get_gemini_response(query: str, passage: str) -> Dict:
    """
    Queries the Gemini API to generate a response, with retries and key rotation.

    Args:
        query (str): The user query.
        passage (str): Relevant passage.

    Returns:
        Dict: Response with extracted answer.
    """
...
def main():
    ...
    db = load_data(documents, metadatas, collection_name)
